...
it('init selected doctors should be zero', function () {
  expect(bookDoctorStore.selectedDoctors.length).toStrictEqual(0);
});

it('init selected symptom should be zero', function () {
  expect(bookDoctorStore.selectedSymptom.length).toStrictEqual(0);
});

it('init favorite should be 11', function () {
  expect(bookDoctorStore.favoriteSymptom.length).toStrictEqual(11);
});

it('init personal symptom should be 8', function () {
  expect(bookDoctorStore.personalSymptom.length).toStrictEqual(8);
});

it('selected doctor should add to selectedDoctors, add element name Ana', function () {
  bookDoctorStore.dispatchSelectedList('Ana');
  expect(bookDoctorStore.selectedDoctors[0]).toStrictEqual('Ana');
});
...
it('selected symptom with first element of constant.symptomList[4]', function () {
  ...
  expect(bookDoctorStore.selectedSymptom[0]).toStrictEqual(
    constantData.symptomList[4],
  );
});
